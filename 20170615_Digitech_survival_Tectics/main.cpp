		...
		while (1)
		{
						...
						if (m_base->roomspace[roomnum - 1].roomtype == 0)
						{
							...
							if (temp == 'y' || temp == 'Y')
							{
								...
								if (m_base->inven[i].cur_cnt >= useitemcnt)
								{
									...
									m_base->roomspace[roomnum - 1].roomlv = 1;
									m_base->roomspace[roomnum - 1].roomtype = 1;
									m_base->roomspace[roomnum - 1].aply_bonus = 5;
					...
					if (roomnum > 0 && 20 >= roomnum)
					{
						...
						if (m_base->roomspace[roomnum - 1].roomtype == 0)
						{
							...
							if (temp == 'y' || temp == 'Y')
							{
								...
								{
									m_base->inven[i].cur_cnt -= useitemcnt;
									m_base->roomspace[roomnum - 1].roomtype = 2;
									m_base->roomspace[roomnum - 1].roomlv = 1;
									m_base->roomspace[roomnum - 1].aply_bonus = 5;
									printf("건설을 성공적으로 마쳤습니다.\n");
									break;
			...
			case '3':
				while (1)
				{
					...
					if (roomnum > 0 && 20 >= roomnum)
					{
						...
						if (m_base->roomspace[roomnum - 1].roomtype == 0)
						{
							...
							if (temp == 'y' || temp == 'Y')
							{
								...
								{
									m_base->inven[i].cur_cnt -= useitemcnt;
									m_base->roomspace[roomnum - 1].roomtype = 3;
									m_base->roomspace[roomnum - 1].roomlv = 1;
									m_base->roomspace[roomnum - 1].aply_bonus = 5;
									printf("건설을 성공적으로 마쳤습니다.\n");
									break;
					...
					printf("저장공간 건설을 그만두시겠습니까? (그만두려면 y 입력)>>");
					bfclear();
					temp = getchar();
					if (temp == 'y' || temp == 'Y') break;
					else printf("계속 진행합니다.\n");
...
{
	printf("좀비가 돌아다니는 디지텍리.. 얼마나 많은 사람들을 구해낼 수 있을까?\n");
	...
	if (base->curlife > 0)
	{
		...
		int rslt = rand() % (((srchbody / 5) + 1) + rand() % ((rqpoint / 5) + 1) + 1);
...
void daytime(leader* user)
{
	...
	printf("\t\t%s의 경로당\n", user->username);
	printf("------------------------------------------------\n\n");
	printf("                생존자 수 : %d/%d\n\n", user->base->curlife, user->base->maxlife);
	printf("------------------------------------------------\n\n");
...
void startmant()
{
	printf("당신은 디지텍리에 갓 부임한 젊은 이장입니다.\n\n");
	printf("이장이 되고 난 뒤, 잘못 들여온 사료로 인해 동물들이 줄지어 죽어나가고,\n\n");
	printf("시체가 살아 움직이며, 디지텍리를 장악하기 시작했습니다.\n\n");
	printf("당신은 10명의 사람들과 함께 경로당에 대피하는 데 성공했습니다.\n\n");
	printf("과연 얼마나 살아남을 수 있을까요? 행운을 빕니다.\n\n\n\n");

	printf("!!주의사항!! 잘못된 문자 입력은 게임에 큰 문제를 발생시킬 수 있습니다. 오타에 주의해주세요.\n\n");
}
...
void main()
{
	...
	while (1)
	{
		...
		realuser.base->inven[0].cur_cnt = 10;
		realuser.base->inven[0].stackmax = 30;
		realuser.base->inven[1].cur_cnt = 10;
